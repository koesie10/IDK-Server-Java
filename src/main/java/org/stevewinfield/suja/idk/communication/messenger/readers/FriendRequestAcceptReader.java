    ...
    public void parse(final Session session, final MessageReader reader) {
        ...
        for (int i = 0; i < amount; i++) {
            ...
            if (session.getPlayerMessenger().getRequests().containsKey(playerId)) {
                ...
                ResultSet userInfo;
                try {
                    userInfo = Bootloader.getStorage().queryParams("SELECT id, nickname, figurecode, motto, gender, stream_enabled FROM players WHERE id = " + playerId).executeQuery();
                    if (userInfo.next()) {
                        final MessengerBuddy buddy = new MessengerBuddy();
                        buddy.set(userInfo);
                        if (session.getPlayerInstance().getInformation().isStreamEnabled()) {
                            session.getFriendStream().broadcastEvent(session, FriendStreamEventType.FRIEND_MADE, FriendStreamLinkType.FRIEND_REQUEST, new String[]{playerId + "", userInfo.getString("nickname")});
                        }
                        ...
                        if (buddy.isOnline()) {
                            ...
                            if (userInfo.getInt("stream_enabled") == 1) {
                                buddy.getSession().getFriendStream().broadcastEvent(buddy.getSession(), FriendStreamEventType.FRIEND_MADE, FriendStreamLinkType.FRIEND_REQUEST, new String[]{session.getPlayerInstance().getInformation().getId() + "", session.getPlayerInstance().getInformation().getPlayerName()});
                            }
        ...
        for (final Session newFriend : newFriends) {
            if (newFriend.isAuthenticated()) {
                final MessengerBuddy buddy = new MessengerBuddy();
                buddy.set(session.getPlayerInstance().getInformation().getId(), session.getPlayerInstance().getInformation().getPlayerName(), session.getPlayerInstance().getInformation().getAvatar(), session.getPlayerInstance().getInformation().getMission());
                newFriend.getPlayerMessenger().getBuddies().put(session.getPlayerInstance().getInformation().getId(), buddy);
                newFriend.getPlayerMessenger().onStatusChanged(session.getPlayerInstance().getInformation().getId(), true);
